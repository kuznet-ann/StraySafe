...
?> <!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?=$eventName?></title><link rel="icon" type="image/x-icon" href="../images/logo.svg"><link rel="stylesheet" href="../css/styles.min.css"></head><body> <?php include('../models/header.php') ?> <section class="breadcrumbs"><div class="container"><div class="breadcrumbs__list"><a href="../index.php" class="breadcrumbs__link title-fz16">Главная</a> <a href="events.php" class="breadcrumbs__link title-fz16">Мероприятия</a> <a href="events-page.php" class="breadcrumbs__link title-fz16"><?=$eventName?></a></div></div></section><section class="events-info"><div class="container"><div class="events-info__info"><img src="../images/events/<?=$image?>" alt="" class="events-info__img"><div class="events-info__wrapper"><h3 class="events-info__title title-fz24"><?=$eventName?></h3> <?php
                            ...
                            ?> <p class="events-info__descr"><?=$descr?></p></div></div> <?php
                if(!empty($_SESSION['role']) && $_SESSION['role'] > 1) {
                    echo '<button class="catalog__btn-edit btn">Изменить запись</button>';
                }
                if(!empty($_SESSION['role']) && $_SESSION['role'] > 2): ?> <form method="post" class="catalog__form-delete"><input class="catalog__btn-delete btn-red btn" type="submit" name="delete" value="Удалить"></form> <?php endif;?> <button class="events-info__btn btn btn-overlay">Принять участие</button></div></section> <?php include('../models/footer.php') ?> <?php include('../models/reg-overlay.php') ?> <div class="overlay"><div class="overlay__contact"><div class="overlay__close"><img src="../images/close.svg" alt="Крестик для закрытия" class="overlay__img"></div><h4 class="overlay__title title-fz20">Обратная связь</h4><p class="overlay__descr">Впишите ваш номер телефона и мы свяжемся с вами с пятницы по понедельник с 12:00 до 18:00</p><form action="" method="post" class="overlay__form"><input data-mask="tel" type="tel" placeholder="Введите номер телефона" name="phone" id="phone" class="overlay__input title-fz16"> <button class="overlay__btn btn">Свяжитесь со мной</button></form></div></div><div class="overlay-change"><div class="overlay__contact active"><div class="overlay__close"><img src="/dist/images/close.svg" alt="Крестик для закрытия" class="overlay__img"></div><h4 class="overlay__title title-fz20">Изменение</h4><form method="POST" class="overlay__form" enctype="multipart/form-data"><input type="text" placeholder="Введите название" name="event-name" id="event-name" class="overlay__input title-fz16" required value="<?=$eventName?>"> <input type="text" placeholder="Введите место" name="place" id="place" class="overlay__input title-fz16" value="<?=$place?>"> <textarea placeholder="Введите описание" name="descr" id="descr" class="overlay__input title-fz16" required><?=$descr?></textarea> <textarea placeholder="Введите краткое описание" name="text" id="text" class="overlay__input title-fz16" required><?=$text?></textarea> <input class="overlay__input title-fz16 overlay__input-small" type="date" name="date" id="date" required value="<?=$date?>"> <select class="overlay__input overlay__input-small overlay__input-margin" name="user" id="user"> <?php
                    ...
                    ?> </select> <input class="overlay__input overlay__input-small" class="overlay__choose" id="picture" name="picture" type="file" required> <input class="overlay__btn btn" type="submit" name="change" value="Изменить"></form></div></div><script src="../js/imask.js"></script><script src="../js/script.js"></script></body></html>